...
@bot.command()
async def userinfo(ctx: commands.Context, user: discord_self.User):
    # In the command signature above, you can see that the `user`
    # parameter is typehinted to `discord_self.User`. This means that
    # during command invocation we will attempt to convert
    # the value passed as `user` to a `discord_self.User` instance.
    # The documentation notes what can be converted, in the case of `discord_self.User`
    ...
    # If the conversion is successful, we will have a `discord_self.User` instance
...
class ChannelOrMemberConverter(commands.Converter):
    async def convert(self, ctx: commands.Context, argument: str):
        ...
        # `discord_self.Member` or `discord_self.TextChannel` instance from the
...
@bot.command()
async def ignore(ctx: commands.Context, target: typing.Union[discord_self.Member, discord_self.TextChannel]):
    ...
    # So, it will attempt to convert whatever is passed to `target` to a `discord_self.Member` instance.
    # If that fails, it will attempt to convert it to a `discord_self.TextChannel` instance.
    ...
    if isinstance(target, discord_self.Member):
        await ctx.send(f'Member found: {target.mention}, adding them to the ignore list.')
    elif isinstance(target, discord_self.TextChannel):  # this could be an `else` but for completeness' sake.
