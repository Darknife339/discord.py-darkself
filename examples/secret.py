...
def create_overwrites(ctx, *objects):
    ...
    A `discord_self.PermissionOverwrite` allows you to determine the permissions
    of an object, whether it be a `discord_self.Role` or a `discord_self.Member`.
    ...
    # for each `discord_self.Role` or `discord_self.Member`.
    overwrites = {obj: discord_self.PermissionOverwrite(view_channel=True) for obj in objects}

    # prevents the default role (@everyone) from viewing the channel
    # if it isn't already allowed to view the channel.
    overwrites.setdefault(ctx.guild.default_role, discord_self.PermissionOverwrite(view_channel=False))

    # makes sure the client is always allowed to view the channel.
    overwrites[ctx.guild.me] = discord_self.PermissionOverwrite(view_channel=True)
...
@secret.command()
@commands.guild_only()
async def text(ctx: commands.Context, name: str, *objects: typing.Union[discord_self.Role, discord_self.Member]):
...
@secret.command()
@commands.guild_only()
async def voice(ctx: commands.Context, name: str, *objects: typing.Union[discord_self.Role, discord_self.Member]):
...
@secret.command()
@commands.guild_only()
async def emoji(ctx: commands.Context, emoji: discord_self.PartialEmoji, *roles: discord_self.Role):
