...
import asyncpg  # asyncpg is not a dependency of the discord_self.py, and is only included here for illustrative purposes.
import discord_self
...
class CustomBot(commands.Bot):
    ...
    async def setup_hook(self) -> None:
        ...
        if self.testing_guild_id:
            guild = discord_self.Object(self.testing_guild_id)
...
async def main():
    ...
    logger = logging.getLogger('discord_self')
    logger.setLevel(logging.INFO)

    handler = logging.handlers.RotatingFileHandler(
        filename='discord_self.log',
        encoding='utf-8',
    ...
    # discord_self.utils.setup_logging(handler=handler, root=False)
